...
1. Joe identifies which agents are needed:
   - **Expert agents** related to the feature (Ford for UI, Dev for code, Spider for infra, etc.)
...
- No ack within 15 min → escalate to Spider (agent may be down)
...
| No start ack within 15 min | Escalate to Spider |
...
| Spec → Approve | 4h | Spider nudges King |
...
| **Spider** | Infrastructure, platform, agent config, deployment |
